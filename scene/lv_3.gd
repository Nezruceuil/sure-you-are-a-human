		...
		if Var.check == false:
			...
			if  chargetime > 0.1:
				...
				if rng.randi_range(1,2) == 1:
					...
					if rng.randi_range(1,2) == 1:
						check.position.y = 172.0
						submit.position.y = 357.0
					else:
						submit.position.y = 172.0
						check.position.y = 330.0
		else:
			fake.visible = false
			charge.visible = false
			human_check.visible = true
	else:
		if timer < Var.score[2]:
			Var.score[2] = round(timer*1000)/1000
			if Var.score[2] < 10:
				Var.advancements[3] = true
			Save._save()
